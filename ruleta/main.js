...
const colores = ["3D44AB", "EB5A4E", "FFFFFF"];
...
const cuatro = {
  nombre: "Hacker de las industrias",
  probabilidad: 11.112,
};
const cinco = {
...
let conceptos = [uno, dos, tres, cuatro, cinco, seis, siete, ocho, nueve];
...
function sortear() {
  ...
  const giroRuleta = (1 - nSorteo) * 360 + 360 * 2; //2 vueltas + lo aleatorio;
...
function ajustarRuleta() {
  ...
  conceptos.forEach((concepto, i) => {
    ...
    nombreElement.style = `width : calc(${concepto.probabilidad} * var(--escala) * 1.5 / 63.5);
